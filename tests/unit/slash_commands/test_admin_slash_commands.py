...
class TestAdminSlashCommands:
    ...
    @pytest.fixture
    def admin_slash_commands(self, mock_bot, mock_ip_service, mock_storage, 
                           mock_config, mock_admin_commands_handler):
    ...
    def test_initialization(self, admin_slash_commands, mock_bot, mock_ip_service, 
                           mock_storage, mock_config, mock_admin_commands_handler):
        ...
        assert admin_slash_commands.admin_commands_handler == mock_admin_commands_handler
...
class TestStopSlashCommand:
    ...
    async def test_stop_slash_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful bot stop command."""
        await admin_slash_commands.stop_slash(mock_interaction_admin)
        ...
        assert mock_interaction_admin.response.send_message.call_args[1]["ephemeral"] is True
        
        # Check that stop callback was called
        admin_slash_commands.stop_callback.assert_called_once()
    
    async def test_stop_slash_permission_denied(self, admin_slash_commands, mock_interaction_non_admin):
        """Test stop command with insufficient permissions."""
        await admin_slash_commands.stop_slash(mock_interaction_non_admin)
    ...
    async def test_stop_slash_exception_handling(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_stop_slash_interaction_already_responded(self, admin_slash_commands, mock_interaction_admin):
        """Test stop command when interaction is already responded to."""
        admin_slash_commands.stop_callback.side_effect = Exception("Test error")
        mock_interaction_admin.response.send_message.side_effect = [
            None,  # First call succeeds
            discord.InteractionResponded()  # Second call fails
        ]
...
class TestConfigSlashCommands:
    ...
    async def test_config_show_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful config show command."""
        await admin_slash_commands.config_show_slash(mock_interaction_admin, None)
    ...
    async def test_config_show_with_field(self, admin_slash_commands, mock_interaction_admin):
        """Test config show command with specific field."""
        await admin_slash_commands.config_show_slash(mock_interaction_admin, "check_interval")
        
        config_handler = admin_slash_commands.admin_commands_handler.get_handler_for_command("config")
    ...
    async def test_config_show_permission_denied(self, admin_slash_commands, mock_interaction_non_admin):
        """Test config show command with insufficient permissions."""
        await admin_slash_commands.config_show_slash(mock_interaction_non_admin, None)
        
        mock_interaction_non_admin.response.send_message.assert_called_once()
        call_args = mock_interaction_non_admin.response.send_message.call_args[0][0]
        assert "❌ You need administrator permissions" in call_args
    
    async def test_config_show_handler_failure(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_config_show_no_handler(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_config_set_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful config set command."""
        await admin_slash_commands.config_set_slash(mock_interaction_admin, "check_interval", "60")
    ...
    async def test_config_set_permission_denied(self, admin_slash_commands, mock_interaction_non_admin):
        """Test config set command with insufficient permissions."""
        await admin_slash_commands.config_set_slash(mock_interaction_non_admin, "field", "value")
        
        mock_interaction_non_admin.response.send_message.assert_called_once()
        call_args = mock_interaction_non_admin.response.send_message.call_args[0][0]
        assert "❌ You need administrator permissions" in call_args
    
    async def test_config_set_handler_failure(self, admin_slash_commands, mock_interaction_admin):
        """Test config set command when handler fails."""
        config_handler = admin_slash_commands.admin_commands_handler.get_handler_for_command("config")
        config_handler._handle_config_set.return_value = False
        
        await admin_slash_commands.config_set_slash(mock_interaction_admin, "field", "value")
        
        mock_interaction_admin.followup.send.assert_called_once()
        call_args = mock_interaction_admin.followup.send.call_args[0][0]
        assert "❌ Failed to update configuration field `field`" in call_args
    
    async def test_config_list_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful config list command."""
        await admin_slash_commands.config_list_slash(mock_interaction_admin)
    ...
    async def test_config_save_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful config save command."""
        await admin_slash_commands.config_save_slash(mock_interaction_admin)
    ...
    async def test_config_reload_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful config reload command."""
        await admin_slash_commands.config_reload_slash(mock_interaction_admin)
    ...
    async def test_config_exception_handling(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_config_interaction_expired(self, admin_slash_commands, mock_interaction_admin):
...
class TestAPISlashCommands:
    ...
    async def test_api_list_success(self, admin_slash_commands, mock_interaction_admin):
        ...
        await admin_slash_commands.api_list_slash(mock_interaction_admin)
    ...
    async def test_api_list_permission_denied(self, admin_slash_commands, mock_interaction_non_admin):
        """Test API list command with insufficient permissions."""
        await admin_slash_commands.api_list_slash(mock_interaction_non_admin)
    ...
    async def test_api_add_success(self, admin_slash_commands, mock_interaction_admin):
        ...
        await admin_slash_commands.api_add_slash(
        ...
        api_handler = admin_slash_commands.admin_commands_handler.get_handler_for_command("api")
        api_handler.handle_command.assert_called_once()
        call_args = api_handler.handle_command.call_args[0][1]
        assert call_args == ["api", "add", "TestAPI", "https://test.example.com", "json", "ip"]
    
    async def test_api_add_auto_format(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_api_add_text_format(self, admin_slash_commands, mock_interaction_admin):
        ...
        api_handler = admin_slash_commands.admin_commands_handler.get_handler_for_command("api")
        call_args = api_handler.handle_command.call_args[0][1]
        assert call_args == ["api", "add", "TestAPI", "https://test.example.com", "text"]
    
    async def test_api_remove_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful API remove command."""
        await admin_slash_commands.api_remove_slash(mock_interaction_admin, "test_api")
    ...
    async def test_api_toggle_enable_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful API enable command."""
        await admin_slash_commands.api_toggle_slash(mock_interaction_admin, "test_api", "enable")
    ...
    async def test_api_toggle_disable_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful API disable command."""
        await admin_slash_commands.api_toggle_slash(mock_interaction_admin, "test_api", "disable")
    ...
    async def test_api_test_success(self, admin_slash_commands, mock_interaction_admin):
        ...
        await admin_slash_commands.api_test_slash(mock_interaction_admin, "test_api")
    ...
    async def test_api_stats_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful API stats command."""
        await admin_slash_commands.api_stats_slash(mock_interaction_admin)
    ...
    async def test_api_handler_failure(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_api_exception_handling(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_api_interaction_expired(self, admin_slash_commands, mock_interaction_admin):
...
class TestCacheSlashCommands:
    ...
    async def test_cache_show_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful cache show command."""
        await admin_slash_commands.cache_show_slash(mock_interaction_admin)
    ...
    async def test_cache_show_permission_denied(self, admin_slash_commands, mock_interaction_non_admin):
        """Test cache show command with insufficient permissions."""
        await admin_slash_commands.cache_show_slash(mock_interaction_non_admin)
        
        mock_interaction_non_admin.response.send_message.assert_called_once()
        call_args = mock_interaction_non_admin.response.send_message.call_args[0][0]
        assert "❌ You need administrator permissions" in call_args
    
    async def test_cache_clear_all_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful cache clear all command."""
        await admin_slash_commands.cache_clear_slash(mock_interaction_admin, None)
        ...
        # Check that handler was called with correct arguments
        cache_handler = admin_slash_commands.admin_commands_handler.get_handler_for_command("cache")
    ...
    async def test_cache_clear_namespace_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful cache clear namespace command."""
        await admin_slash_commands.cache_clear_slash(mock_interaction_admin, "ip_check")
    ...
    async def test_cache_handler_failure(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_cache_exception_handling(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_cache_interaction_expired(self, admin_slash_commands, mock_interaction_admin):
...
class TestQueueSlashCommands:
    ...
    async def test_queue_status_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful queue status command."""
        await admin_slash_commands.queue_status_slash(mock_interaction_admin)
        ...
        # Check that handler was called with correct arguments
        queue_handler = admin_slash_commands.admin_commands_handler.get_handler_for_command("queue")
    ...
    async def test_queue_status_permission_denied(self, admin_slash_commands, mock_interaction_non_admin):
        """Test queue status command with insufficient permissions."""
        await admin_slash_commands.queue_status_slash(mock_interaction_non_admin)
        
        mock_interaction_non_admin.response.send_message.assert_called_once()
        call_args = mock_interaction_non_admin.response.send_message.call_args[0][0]
        assert "❌ You need administrator permissions" in call_args
    
    async def test_queue_clear_success(self, admin_slash_commands, mock_interaction_admin):
        """Test successful queue clear command."""
        await admin_slash_commands.queue_clear_slash(mock_interaction_admin)
    ...
    async def test_queue_handler_failure(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_queue_exception_handling(self, admin_slash_commands, mock_interaction_admin):
    ...
    async def test_queue_interaction_expired(self, admin_slash_commands, mock_interaction_admin):
