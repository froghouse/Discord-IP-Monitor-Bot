...
class TestApiHandler:
    ...
    @pytest.fixture
    def api_handler(self, mock_client, mock_ip_service, mock_storage, mock_stop_callback, mock_config):
    ...
    def mock_api_manager(self):
        """Mock IP API manager."""
        with patch('ip_monitor.commands.admin_commands.api_handler.ip_api_manager') as mock_manager:
    ...
    async def test_handle_command_api_command(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_command_no_permissions(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_command_no_args_shows_help(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_command_list_subcommand(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_command_add_subcommand(self, api_handler, mock_message, mock_api_manager):
        """Test api add subcommand."""
        mock_message.author.guild_permissions.administrator = True
        
        result = await api_handler._handle_api_command(mock_message, ['add', 'TestAPI', 'https://api.test.com'])
        assert result is True

    async def test_handle_api_command_invalid_subcommand(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_command_exception(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_list_no_apis(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_list_with_apis(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
    ...
    async def test_handle_api_add_insufficient_args(self, api_handler, mock_message, mock_api_manager):
        """Test api add with insufficient arguments."""
        result = await api_handler._handle_api_add(mock_message, ['add', 'name'])
        assert result is True

    async def test_handle_api_add_success(self, api_handler, mock_message, mock_api_manager):
        ...
        result = await api_handler._handle_api_add(mock_message, ['add', 'TestAPI', 'https://api.test.com'])
        assert result is True
        mock_api_manager.add_api.assert_called_once()

    async def test_handle_api_add_with_format_and_field(self, api_handler, mock_message, mock_api_manager):
        """Test api add with format and field."""
        mock_api_manager.get_api.return_value = None
        mock_api_manager.add_api.return_value = True
        
        result = await api_handler._handle_api_add(mock_message, ['add', 'TestAPI', 'https://api.test.com', 'json', 'ip'])
        assert result is True

    async def test_handle_api_add_invalid_format(self, api_handler, mock_message, mock_api_manager):
        """Test api add with invalid format."""
        result = await api_handler._handle_api_add(mock_message, ['add', 'TestAPI', 'https://api.test.com', 'invalid'])
        assert result is True

    async def test_handle_api_add_already_exists(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
    ...
    async def test_handle_api_add_exception(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_remove_insufficient_args(self, api_handler, mock_message, mock_api_manager):
        """Test api remove with insufficient arguments."""
        result = await api_handler._handle_api_remove(mock_message, ['remove'])
        assert result is True

    async def test_handle_api_remove_success(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
        """Test successful api remove."""
        mock_api_manager.get_api.return_value = mock_api_endpoint
        mock_api_manager.remove_api.return_value = True
        
        result = await api_handler._handle_api_remove(mock_message, ['remove', 'test_api'])
        assert result is True
        mock_api_manager.remove_api.assert_called_once_with('test_api')

    async def test_handle_api_remove_not_found(self, api_handler, mock_message, mock_api_manager):
        """Test api remove when API not found."""
        mock_api_manager.get_api.return_value = None
        
        result = await api_handler._handle_api_remove(mock_message, ['remove', 'nonexistent'])
        assert result is True

    async def test_handle_api_enable_success(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
        """Test successful api enable."""
        mock_api_manager.get_api.return_value = mock_api_endpoint
        mock_api_manager.enable_api.return_value = True
        
        result = await api_handler._handle_api_enable(mock_message, ['enable', 'test_api'])
        assert result is True
        mock_api_manager.enable_api.assert_called_once_with('test_api')

    async def test_handle_api_disable_success(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
        """Test successful api disable."""
        mock_api_manager.get_api.return_value = mock_api_endpoint
        mock_api_manager.disable_api.return_value = True
        
        result = await api_handler._handle_api_disable(mock_message, ['disable', 'test_api'])
        assert result is True
        mock_api_manager.disable_api.assert_called_once_with('test_api')

    async def test_handle_api_test_insufficient_args(self, api_handler, mock_message, mock_api_manager):
        """Test api test with insufficient arguments."""
        result = await api_handler._handle_api_test(mock_message, ['test'])
        assert result is True

    async def test_handle_api_test_success(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
        ...
        with patch.object(api_handler, '_test_single_api') as mock_test:
            ...
            result = await api_handler._handle_api_test(mock_message, ['test', 'test_api'])
            assert result is True
            mock_test.assert_called_once_with(mock_api_endpoint)

    async def test_handle_api_test_failure(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
    ...
    async def test_handle_api_priority_insufficient_args(self, api_handler, mock_message, mock_api_manager):
        """Test api priority with insufficient arguments."""
        result = await api_handler._handle_api_priority(mock_message, ['priority', 'test_api'])
        assert result is True

    async def test_handle_api_priority_invalid_number(self, api_handler, mock_message, mock_api_manager):
        """Test api priority with invalid number."""
        result = await api_handler._handle_api_priority(mock_message, ['priority', 'test_api', 'invalid'])
        assert result is True

    async def test_handle_api_priority_success(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
        """Test successful api priority."""
        mock_api_manager.get_api.return_value = mock_api_endpoint
        mock_api_manager.set_api_priority.return_value = True
        
        result = await api_handler._handle_api_priority(mock_message, ['priority', 'test_api', '5'])
        assert result is True
        mock_api_manager.set_api_priority.assert_called_once_with('test_api', 5)

    async def test_handle_api_stats_no_stats(self, api_handler, mock_message, mock_api_manager):
    ...
    async def test_handle_api_stats_with_stats(self, api_handler, mock_message, mock_api_manager, mock_api_endpoint):
    ...
    async def test_test_single_api_auto_detect_json(self, api_handler, mock_api_endpoint):
