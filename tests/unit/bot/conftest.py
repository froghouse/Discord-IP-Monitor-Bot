...
def mock_bot_message_queue():
    ...
    queue.get_status = Mock(return_value={
        "enabled": True,
        "processing": False,
        "queued_messages": 0,
        "total_processed": 0,
...
def mock_bot_service_health():
    ...
    # Status reporting
    health.get_status = Mock(return_value={
        "level": "NORMAL",
        "score": 100,
        "issues": [],
        "services": {
            "ip_service": {"status": "healthy", "last_check": "2023-01-01T12:00:00Z"},
            "discord_api": {"status": "healthy", "last_check": "2023-01-01T12:00:00Z"},
...
def mock_bot_rate_limiter():
    ...
    # Status reporting
    limiter.get_status = AsyncMock(return_value={
        "period": 300,
        "max_calls": 10,
        "active_calls": 0,
        "remaining_calls": 10,
...
def mock_bot_discord_rate_limiter():
    ...
    # Status reporting
    limiter.get_status = Mock(return_value={
        "active_limits": {},
        "recent_429s": 0,
        "total_requests": 0,
        "total_limited": 0,
...
def mock_bot_instance(
    ...
    with patch("ip_monitor.bot.commands.Bot", return_value=mock_discord_client):
        with patch("ip_monitor.bot.discord.Intents.default", return_value=mock_discord_intents):
            with patch("ip_monitor.bot.IPService", return_value=mock_bot_ip_service):
                with patch("ip_monitor.bot.SQLiteIPStorage", return_value=mock_bot_storage):
                    with patch("ip_monitor.bot.AsyncRateLimiter", return_value=mock_bot_rate_limiter):
                        with patch("ip_monitor.bot.DiscordRateLimiter", return_value=mock_bot_discord_rate_limiter):
                            with patch("ip_monitor.bot.IPCommands", return_value=mock_bot_commands):
                                with patch("ip_monitor.bot.AdminCommandRouter", return_value=mock_bot_commands):
                                    with patch("ip_monitor.bot.message_queue", mock_bot_message_queue):
                                        with patch("ip_monitor.bot.service_health", mock_bot_service_health):
                                            ...
                                            bot.discord_rate_limiter = mock_bot_discord_rate_limiter
