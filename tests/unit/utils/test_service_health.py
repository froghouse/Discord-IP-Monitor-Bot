...
class TestServiceHealthRecording:
    ...
    def test_record_success_unknown_service(self, health_monitor, mock_time):
        ...
        with patch('ip_monitor.utils.service_health.logger') as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with("Unknown service: unknown_service")
    ...
    def test_record_failure_unknown_service(self, health_monitor, mock_time):
        ...
        with patch('ip_monitor.utils.service_health.logger') as mock_logger:
            health_monitor.record_failure('unknown_service', 'Test error', 'test_operation')
            mock_logger.warning.assert_called_once_with("Unknown service: unknown_service")
...
class TestDegradationLevelTransitions:
    ...
    def test_multiple_critical_service_failures_forces_critical(self, health_monitor, mock_time):
...
class TestSystemHealthReporting:
    ...
    def test_get_system_health_degraded(self, health_monitor, mock_time):
        ...
        assert system_health['degradation_description'] == 'Multiple service failures, core functionality only'
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_degradation_level_comparison(self, health_monitor):
        ...
        with patch('ip_monitor.utils.service_health.logger') as mock_logger:
            health_monitor.force_degradation_level(DegradationLevel.MINOR, "improvement")
            # Should call info for improvement (severe > minor alphabetically)
            calls = [call for call in mock_logger.info.call_args_list if 'decreased' in str(call)]
            assert len(calls) > 0
        ...
        with patch('ip_monitor.utils.service_health.logger') as mock_logger:
            health_monitor.force_degradation_level(DegradationLevel.SEVERE, "degradation")
            # Should call warning for degradation (severe > minor alphabetically)
            calls = [call for call in mock_logger.warning.call_args_list if 'increased' in str(call)]
            assert len(calls) > 0
    ...
    def test_service_status_transitions_with_logging(self, health_monitor, mock_time):
        ...
        with patch('ip_monitor.utils.service_health.logger') as mock_logger:
            ...
            recovery_calls = [call for call in mock_logger.info.call_args_list 
