...
class TestIntelligentCacheOperations:
    ...
    def test_set_with_cache_type(self, cache):
        ...
        cache.set(
            "test_namespace", "test_id", "test_value", cache_type=CacheType.DNS_LOOKUP
        )
...
class TestCacheStatistics:
    ...
    def test_statistics_thread_safety(self, cache):
        ...
        def worker():
            for i in range(10):
                cache.set(
                    f"thread_{threading.current_thread().ident}", f"key{i}", f"value{i}"
                )
...
class TestCacheInvalidation:
    ...
    def test_invalidate_entire_namespace(self, cache):
        ...
        assert (
            cache.get("namespace2", "key1") == "value3"
...
class TestCacheThreadSafety:
    ...
    def test_statistics_thread_safety(self, cache):
        ...
        def stats_worker():
            for i in range(100):
                cache.set(
                    f"test_{threading.current_thread().ident}", f"key{i}", f"value{i}"
                )
