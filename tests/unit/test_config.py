...
class TestAppConfigValidationErrorHandling:
    ...
    def test_validate_config_value_float_with_scientific_notation(self, config):
        ...
        result = config.validate_config_value('circuit_breaker_recovery_timeout', '1.5e2')
    ...
    def test_validate_config_value_extreme_values(self, config):
        ...
        result = config.validate_config_value('circuit_breaker_recovery_timeout', '3600.0')
        assert result['valid'] is True
        assert result['converted_value'] == 3600.0
        
        # Test minimum float value
        result = config.validate_config_value('circuit_breaker_recovery_timeout', '10.0')
        assert result['valid'] is True
    ...
    def test_get_runtime_configurable_fields_completeness(self, config):
        ...
        expected_fields = [
            'check_interval', 'max_retries', 'retry_delay', 'concurrent_api_checks',
            'circuit_breaker_enabled', 'circuit_breaker_failure_threshold',
            'circuit_breaker_recovery_timeout', 'rate_limit_period', 'max_checks_per_period',
            'ip_history_size', 'startup_message_enabled', 'custom_apis_enabled',
            'message_queue_enabled', 'message_queue_max_size', 'message_queue_max_age_hours',
            'message_queue_batch_size', 'message_queue_process_interval',
            'connection_pool_size', 'connection_pool_max_keepalive',
            'connection_timeout', 'read_timeout', 'cache_enabled', 'cache_ttl',
            'cache_max_memory_size', 'cache_stale_threshold', 'cache_cleanup_interval'
        ]
        
        for field in expected_fields:
            assert field in fields, f"Field {field} not found in runtime configurable fields"
    ...
    def test_get_runtime_configurable_fields_field_metadata(self, config):
        ...
        for field_name, field_info in fields.items():
            ...
            if field_info['type'] in ['int', 'float']:
                assert 'min_value' in field_info, f"Numeric field {field_name} missing min_value"
                assert 'max_value' in field_info, f"Numeric field {field_name} missing max_value"
                assert field_info['min_value'] < field_info['max_value'], f"Field {field_name} has invalid range"
            
            # Boolean fields should not have min/max values
            if field_info['type'] == 'bool':
                assert 'min_value' not in field_info, f"Boolean field {field_name} should not have min_value"
                assert 'max_value' not in field_info, f"Boolean field {field_name} should not have max_value"
    ...
    def test_to_dict_completeness(self, config):
        ...
        expected_fields = [
            'discord_token', 'channel_id', 'check_interval', 'max_retries', 'retry_delay',
            'concurrent_api_checks', 'circuit_breaker_enabled', 'circuit_breaker_failure_threshold',
            'circuit_breaker_recovery_timeout', 'rate_limit_period', 'max_checks_per_period',
            'db_file', 'ip_file', 'ip_history_file', 'ip_history_size', 'startup_message_enabled',
            'testing_mode', 'log_level', 'custom_apis_enabled', 'api_config_file',
            'message_queue_enabled', 'message_queue_max_size', 'message_queue_max_age_hours',
            'message_queue_batch_size', 'message_queue_process_interval',
            'connection_pool_size', 'connection_pool_max_keepalive',
...
class TestAppConfigEdgeCases:
    ...
    @patch('ip_monitor.config.load_dotenv', side_effect=Exception("Failed to load .env"))
...
class TestAppConfigFileHandlingEdgeCases:
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.dump', side_effect=TypeError("Object of type X is not JSON serializable"))
    @patch('ip_monitor.config.logger')
    def test_save_to_file_json_encode_error(self, mock_logger, mock_json_dump, mock_file, config):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.dump')
    def test_save_to_file_with_existing_file_timestamp(self, mock_json_dump, mock_file, mock_getmtime, mock_exists, config):
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', new_callable=mock_open, read_data='{"corrupted": true, "max_retries": null}')
    @patch('json.load')
    def test_load_from_file_with_null_values(self, mock_json_load, mock_file, mock_exists, config):
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', new_callable=mock_open, read_data='{"unknown_field": "value", "max_retries": 5}')
    @patch('json.load')
    @patch('ip_monitor.config.logger')
    def test_load_from_file_with_unknown_fields(self, mock_logger, mock_json_load, mock_file, mock_exists, config):
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', new_callable=mock_open, read_data='{"max_retries": "not_a_number"}')
    @patch('json.load')
    def test_load_from_file_with_wrong_type_values(self, mock_json_load, mock_file, mock_exists, config):
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', side_effect=UnicodeDecodeError("utf-8", b'', 0, 1, "Invalid character"))
    @patch('ip_monitor.config.logger')
    def test_load_from_file_unicode_decode_error(self, mock_logger, mock_file, mock_exists, config):
    ...
    @patch('json.load')
    def test_load_from_file_empty_json_object(self, mock_json_load, mock_file, mock_exists, config):
    ...
    @patch('json.load')
    @patch('ip_monitor.config.logger')
    def test_load_from_file_json_array_instead_of_object(self, mock_logger, mock_json_load, mock_file, mock_exists, config):
...
class TestAppConfigRuntimeUpdates:
    ...
    def test_runtime_config_field_validation_and_update(self, config):
        ...
        update_result = config.update_field('max_retries', validation_result['converted_value'])
    ...
    def test_runtime_config_boolean_field_updates(self, config):
        ...
        boolean_fields = [
            'concurrent_api_checks', 'circuit_breaker_enabled', 'startup_message_enabled',
            'custom_apis_enabled', 'message_queue_enabled', 'cache_enabled'
        ]
        ...
        for field in boolean_fields:
            ...
            # Validate and update
            validation_result = config.validate_config_value(field, str(new_value).lower())
            assert validation_result['valid'] is True
            assert validation_result['converted_value'] == new_value
            
            update_result = config.update_field(field, validation_result['converted_value'])
            assert update_result is True
    ...
    def test_runtime_config_numeric_field_updates(self, config):
        ...
        for field, new_value in numeric_test_cases:
            ...
            assert validation_result['valid'] is True, f"Validation failed for {field} with value {new_value}"
            assert validation_result['converted_value'] == new_value
    ...
    def test_runtime_config_validation_prevents_invalid_updates(self, config):
        ...
        for field, invalid_value in invalid_test_cases:
            ...
            assert validation_result['valid'] is False, f"Validation should have failed for {field} with value {invalid_value}"
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.dump')
    def test_runtime_config_persistence_workflow(self, mock_json_dump, mock_file, config):
...
class TestAppConfigMigration:
    ...
    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', new_callable=mock_open, read_data='{"check_interval": 45, "max_retries": 5}')
    @patch('json.load')
    def test_migration_with_partial_config_override(self, mock_json_load, mock_file, mock_exists, base_config):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.load')
    def test_migration_with_new_fields_added(self, mock_json_load, mock_file, mock_exists, base_config):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.load')
    def test_migration_preserves_data_types(self, mock_json_load, mock_file, mock_exists, base_config):
    ...
    def test_migration_validation_after_load(self, base_config):
        ...
        fields_to_validate = [
            ...
            'circuit_breaker_recovery_timeout', 'rate_limit_period', 'max_checks_per_period',
            'ip_history_size', 'startup_message_enabled', 'custom_apis_enabled',
            'cache_enabled', 'cache_ttl', 'cache_stale_threshold'
        ]
        
        for field in fields_to_validate:
            current_value = getattr(base_config, field)
            validation_result = base_config.validate_config_value(field, str(current_value))
            
            assert validation_result['valid'] is True, f"Field {field} with value {current_value} failed validation after migration"
