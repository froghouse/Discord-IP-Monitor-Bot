...
from tests.utils.http_server_mocks import MockIPAPIServer, HTTPMockFixture
...
class TestBotLifecycleHTTPIntegration:
    ...
    async def test_bot_startup_with_http_server(self, http_fixture, bot_instance, mock_config):
    ...
    async def test_periodic_ip_monitoring_with_http(self, http_fixture, bot_instance, storage):
    ...
    async def test_bot_command_handling_with_http(self, http_fixture, bot_instance, mock_discord_client):
    ...
    async def test_admin_command_api_management(self, http_fixture, bot_instance):
        ...
        with patch.object(bot_instance, 'ip_api_config') as mock_api_config:
            mock_api_config.test_api = AsyncMock(return_value={
                "success": True,
                "ip": "203.0.113.1",
                "response_time": 0.1
            })
    ...
    async def test_error_handling_during_monitoring(self, http_fixture, bot_instance, storage):
        ...
        with patch.object(bot_instance.ip_service, 'apis', [
    ...
    async def test_circuit_breaker_behavior_in_monitoring(self, http_fixture, bot_instance):
        ...
        with patch.object(bot_instance.ip_service, 'apis', [f"{failing_server.base_url}/json"]):
    ...
    async def test_message_queue_integration(self, http_fixture, bot_instance):
        ...
        with patch.object(bot_instance, 'message_queue') as mock_queue:
            ...
            mock_queue.get_status = Mock(return_value={
                "queued_messages": 0,
                "processing": True,
                "total_processed": 0,
                "total_failed": 0
    ...
    async def test_bot_shutdown_with_active_http_connections(self, http_fixture, bot_instance):
    ...
    async def test_performance_monitoring_integration(self, http_fixture, bot_instance):
        ...
        with patch.object(bot_instance, 'ip_api_config') as mock_api_config:
            mock_api_config.get_performance_stats = Mock(return_value=[
                {
                    "id": "fast",
                    "name": "Fast API",
                    "success_rate": 100.0,
    ...
    async def test_end_to_end_monitoring_cycle(self, http_fixture, bot_instance, storage):
    ...
    async def test_resilience_under_network_conditions(self, http_fixture, bot_instance):
        ...
        with patch.object(bot_instance.ip_service, 'apis', [f"{unreliable_server.base_url}/json"]):
