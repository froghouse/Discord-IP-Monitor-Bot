...
class TestAdminCommandsHTTPIntegration:
    ...
    async def test_api_add_command_with_http_server(self, http_fixture, admin_router, mock_admin_message):
        ...
        with patch.object(admin_router.api_handler, 'ip_api_config', api_config):
            # Test adding API
            mock_admin_message.content = f"!admin api add TestAPI {server.base_url}/json json ip"
    ...
    async def test_api_test_command_with_http_server(self, http_fixture, admin_router, mock_admin_message):
        """Test API testing command with HTTP server."""
        server = await http_fixture.create_server()
        
        # Create API config with test endpoint
        api_config = IPAPIManager()
        api_config.add_api("TestAPI", f"{server.base_url}/json", ResponseFormat.JSON, "ip")
    ...
    async def test_api_test_command_with_failing_server(self, http_fixture, admin_router, mock_admin_message):
        ...
        api_config = IPAPIManager()
        api_config.add_api("FailingAPI", f"{server.base_url}/json", ResponseFormat.JSON, "ip")
    ...
    async def test_api_stats_command_with_http_performance(self, http_fixture, admin_router, mock_admin_message):
        ...
        api_config = IPAPIManager()
        api_config.add_api("FastAPI", f"{fast_server.base_url}/json", ResponseFormat.JSON, "ip")
        api_config.add_api("SlowAPI", f"{slow_server.base_url}/json", ResponseFormat.JSON, "ip")
    ...
    async def test_api_list_command_with_performance_data(self, http_fixture, admin_router, mock_admin_message):
    ...
    async def test_cache_integration_with_http_apis(self, http_fixture, admin_router, mock_admin_message):
        """Test cache integration with HTTP APIs."""
        server = await http_fixture.create_server()
        
        # Mock cache system
        mock_cache = Mock()
        mock_cache.get_status = Mock(return_value={
            "memory_entries": 5,
            "hit_rate": 0.8,
    ...
    async def test_config_runtime_updates_affecting_http_behavior(self, http_fixture, admin_router, mock_admin_message):
    ...
    async def test_api_cluster_management(self, http_fixture, admin_router, mock_admin_message):
        ...
        with patch.object(admin_router.api_handler, 'ip_api_config', api_config):
            ...
            for i, url in enumerate(server_urls):
                mock_admin_message.content = f"!admin api add ClusterAPI{i} {url} json ip"
    ...
    async def test_api_failover_management(self, http_fixture, admin_router, mock_admin_message):
        ...
        api_config = IPAPIManager()
        api_config.add_api("PrimaryAPI", f"{primary_server.base_url}/json", ResponseFormat.JSON, "ip")
        api_config.add_api("BackupAPI", f"{backup_server.base_url}/json", ResponseFormat.JSON, "ip")
    ...
    async def test_api_performance_monitoring_over_time(self, http_fixture, admin_router, mock_admin_message):
        ...
        # Create API config
        api_config = IPAPIManager()
        api_config.add_api("MonitoredAPI", f"{server.base_url}/json", ResponseFormat.JSON, "ip")
    ...
    async def test_rate_limiting_detection_and_management(self, http_fixture, admin_router, mock_admin_message):
        ...
        # Create API config
        api_config = IPAPIManager()
        api_config.add_api("RateLimitedAPI", f"{server.base_url}/rate_limit", ResponseFormat.JSON, "ip")
        ...
        with patch.object(admin_router.api_handler, 'ip_api_config', api_config):
            ...
            for i in range(4):
                ...
                if i >= 2:  # Should be rate limited
                    assert "rate limit" in response.lower() or "failed" in response.lower()
        
        # Verify server received requests
        assert server.get_request_count() >= 2
    
    async def test_api_security_and_error_handling(self, http_fixture, admin_router, mock_admin_message):
        ...
        with patch.object(admin_router.api_handler, 'ip_api_config', api_config):
            ...
            for url in malicious_urls:
                mock_admin_message.content = f"!admin api add MaliciousAPI {url} json ip"
                mock_admin_message.channel.send.reset_mock()
    ...
    async def test_comprehensive_admin_workflow(self, http_fixture, admin_router, mock_admin_message):
        ...
        with patch.object(admin_router.api_handler, 'ip_api_config', api_config):
            # Step 1: Add APIs
            mock_admin_message.content = f"!admin api add MainAPI {api_server.base_url}/json json ip"
            await admin_router.handle_admin_command(mock_admin_message)
            
            mock_admin_message.content = f"!admin api add BackupAPI {backup_server.base_url}/json json ip"
            mock_admin_message.channel.send.reset_mock()
            await admin_router.handle_admin_command(mock_admin_message)
