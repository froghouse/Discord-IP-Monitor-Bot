...
class PerformanceTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if exc_type is None:
            self.logger.debug(f"{self.operation_name} completed in {self.duration:.3f}s")
        else:
            self.logger.debug(f"{self.operation_name} failed after {self.duration:.3f}s: {exc_val}")
...
class PerformanceBenchmark:
    ...
    async def benchmark_async_operation(
        ...
        **kwargs
        ...
        self.logger.info(f"Iterations: {iterations}, Concurrency: {concurrent_operations}")
...
class LoadTestScenario:
    ...
    async def run_load_test(self, *args, **kwargs) -> Dict[str, PerformanceMetrics]:
        ...
        self.logger.info(f"Ramp-up: {self.ramp_up_duration}s, Sustained: {self.sustained_duration}s")
        ...
        ramp_up_iterations = int(self.ramp_up_duration * (self.target_ops_per_second or 1))
        ...
        sustained_iterations = int(self.sustained_duration * (self.target_ops_per_second or 1))
...
class MemoryProfiler:
    ...
    def start_profiling(self):
        ...
        try:
            ...
            self.logger.info(f"Memory profiling started. Initial memory: {self.initial_memory:.2f} MB")
        except ImportError:
    ...
    def stop_profiling(self) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.info(f"Memory profiling completed. Peak memory: {self.peak_memory:.2f} MB")
            return memory_stats
...
class PerformanceTestSuite:
    ...
    async def run_complete_suite(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["performance_metrics"]["ip_api"] = await benchmark_ip_api_performance(
                self.ip_service,
                iterations=25,
            ...
            results["performance_metrics"]["database"] = await benchmark_database_performance(
                self.storage,
                iterations=50,
            ...
            self.logger.info(f"Performance test suite completed in {results['test_duration']:.2f}s")
    ...
    def generate_performance_report(self, results: Dict[str, Any]) -> str:
        ...
        if results.get("memory_profile"):
            ...
            report.append(f"  Initial Memory: {memory.get('initial_memory_mb', 0):.2f} MB")
            ...
            report.append(f"  Memory Increase: {memory.get('memory_increase_mb', 0):.2f} MB")
            report.append("")
            ...
            for category, metrics in results["performance_metrics"].items():
                ...
                if isinstance(metrics, dict):
                    ...
                    for subcategory, metric in metrics.items():
                        ...
                        report.extend([f"    {line}" for line in str(metric).split('\n')])
                else:
                    # Single metric
