...
class DatabaseTestManager:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        self.backup: DatabaseBackup | None = None
    ...
    def backup_database(self, test_name: str) -> DatabaseBackup | None:
        ...
        backup_fd, backup_path = tempfile.mkstemp(
            suffix=".db", prefix=f"backup_{test_name}_"
        )
        os.close(backup_fd)
    ...
    def clear_database(self, db_path: str | None = None) -> bool:
    ...
    @contextmanager
    def transaction_rollback_test(self, db_path: str | None = None):
...
def ip_history_fixture(db_path: str, history_count: int = 10, **kwargs) -> None:
    ...
    manager.seed_test_data(current_ip=history[-1].ip, ip_history=history_data)
...
def large_dataset_fixture(db_path: str, record_count: int = 1000, **kwargs) -> None:
    ...
    bulk_data = DatabaseRecordGenerator.generate_bulk_test_data(
        current_ip_count=1, history_count=record_count
    )
...
@contextmanager
def isolated_database_test(test_name: str = "test", fixture: str | None = None):
...
def performance_test_database(
    test_name: str = "performance_test", initial_records: int = 100
):
    ...
    default_fixture_manager.apply_fixture(
        "large_dataset", db_path, record_count=initial_records
    )
...
async def async_cleanup_test_database(db_path: str) -> None:
    ...
    await asyncio.get_event_loop().run_in_executor(None, cleanup_test_database, db_path)
...
class AsyncDatabaseTestManager:
    ...
    async def clear_database(self, db_path: str | None = None) -> bool:
